...
public class NotifyDefault {
    ...
    public NotifyDefault(Context context) {
    ...
    public NotifyDefault(Context context, int source) {
        this.source = source;
        this.context = context;
        imageLoader = new ImageLoader(context);
        sp = PreferenceManager.getDefaultSharedPreferences(context.getApplicationContext());
        if (source == AppConstants.TYPE_WIFI) {
            //    Log.i("NotifyDefault", "type wifi");
            NotifyWifi();
        } else if (source == AppConstants.TYPE_MOBILE) {
            //   Log.i("NotifyDefault", "type mobile");
            NotifyData();
        }
    }
    ...
    private void NotifyWifi() {
        ...
        if (file.exists() == false) {
            // Log.d("NotifyDefault", "To Download: " + AppConstants.url_weather_icon + sp.getString(context.getString(R.string.icon_weather), ""));
...
//            notification.defaults |= Notification.DEFAULT_SOUND; // Sound
    ...
    private void getWifiData() {
        ...
        if (Build.VERSION.SDK_INT >= 21)
            channel = Integer.toString(new WifiUtil(context).getWifiChannel());
        else
            channel = "";
